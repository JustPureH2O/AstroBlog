...
(function() {
        if (window.__ecRunButtonInstalled) return;
        window.__ecRunButtonInstalled = true;

        const SUPPORTED_LANGS = ['javascript','typescript','python','html','css','jsx','tsx','vue','svelte','go','rust','c++','java','c'];
        const LANG_PARAM_MAP = {
          javascript:'js', typescript:'ts', python:'py', html:'html', css:'css',
          jsx:'jsx', tsx:'tsx', vue:'vue', svelte:'svelte', go:'go', rust:'rs', 
		  cpp:'c++', java:'java', c:'c',
        };

        function getCodeFromBlock(block) {
          const codeLine = block.querySelectorAll('.ec-line .code');
		  let ret = "";
		  for (let a of codeLine) {
        ...
        function getLanguageFromBlock(block) {
          ...
          if (pre) return pre.dataset.language || '';
          return '';
        }

        function createRunButton(code, lang) {
          const btn = document.createElement('button');
          btn.className = 'ec-run-button';
          btn.type = 'button';
          btn.setAttribute('aria-label', '运行此代码');
          ...
          btn.addEventListener('click', function(e) {
            ...
            if (!code) return;
            const param = (lang === 'c++' ? 'cpp' : LANG_PARAM_MAP[lang]) || 'text';
            const url = 'https://livecodes.io/?' + param + '=' + encodeURIComponent(code) + '&console=open';
        ...
        function addRunButtons() {
          document.querySelectorAll('.expressive-code figure').forEach(figure => {
            // 避免重复添加
            if (figure.querySelector('.ec-run-button')) return;

            const lang = getLanguageFromBlock(figure).toLowerCase();
            if (!SUPPORTED_LANGS.includes(lang)) return;

            const code = getCodeFromBlock(figure);
            if (!code) return;

            // 找复制按钮（通常是最后一个button，或者有特定类）
            let copyBtn = figure.querySelector('.copy button, button[aria-label="Copy code"]');
            // 如果找不到，我们直接追加到末尾
            const btn = createRunButton(code, lang);
